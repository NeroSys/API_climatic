...
function initMap() {
    ...
    $('.map-app__filters').find('select[name="regions"]').on("change", function () {
        ...
        if( placeId !== "all") {
            ...
            geocodePlaceId(geocoder, map, infowindow, 'ChIJjw5wVMHZ0UAREED2iIQGAQA', 6);
            getDistrict(placeId, s2);
            showSortMarkers(fieldsJSON, 'ChIJjw5wVMHZ0UAREED2iIQGAQA', "", "all");
        }
    });
    $('.map-app__filters').find('select[name="areas"]').on("change", function (event) {
        ...
        if( placeId !== "all") {
            ...
            if( curplaceIdRegion !== "all") {
                geocodePlaceId(geocoder, map, infowindow, curplaceIdRegion, 8);
                showSortMarkers(fieldsJSON, curplaceIdRegion, "", "");
            }
            else {
                geocodePlaceId(geocoder, map, infowindow, 'ChIJjw5wVMHZ0UAREED2iIQGAQA', 6);
                showSortMarkers(fieldsJSON, 'ChIJjw5wVMHZ0UAREED2iIQGAQA', "", "all");
...
function setDangerInfo (sFieldId, sStationId, sDangerId, jsonFieldsObj) {
    ...
    infoDescription.append("<p>" + oDangerInfo.name + ", &nbsp &nbsp" + oDangerInfo.percent + "</p>");
