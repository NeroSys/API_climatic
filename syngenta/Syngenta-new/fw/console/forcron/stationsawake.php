        ...
        foreach ($row as $key => $value) {
            ...
            if (in_array($ch, array_keys($ch_keys1))) {
                ...
                if ($ch_value1[$ch] < $value) {
                    $resultvalue = $ch_value1[$ch];
                } else {
                    $resultvalue = round(100*$value/$ch_value1[$ch]);
                }

                $sthz = $zdbh1->prepare('insert into  `stations_threats` (`station_id`,`threat_id`,`value`,`time`,`old`) values (?,?,?,?,?);');

              //  $sthz->execute([$stations_ids1[$station1['f_name']], $ch_keys1[$ch], $resultvalue,date('d.m.y H:00', $time),$old]);
...
foreach  ($stations as $station) {
    ...
    //$station_data = & $soap_station->PesslSOAPStationDataGetFromDate(date('Y-m-d',time()-(24*60*60)).' 06:00:00') or
        ...
        $station_data = &$soap_station->PesslSOAPStationDataGetFromDate(date('Y-m-d H:00:00', time()-60*60));

        if ($station_data) {

            process($station_data,$soap_station,$ch_keys,$ch_value,$zdbh,$stations_ids,$station,$chs_by_culture, time()-60*60,0);

        }elseif($station_data = &$soap_station->PesslSOAPStationDataGetFromDate(date('Y-m-d H:00:00', time()-2*60*60)) ){

            process($station_data,$soap_station,$ch_keys,$ch_value,$zdbh,$stations_ids,$station,$chs_by_culture, time()-2*60*60,1);

        }elseif($station_data = &$soap_station->PesslSOAPStationDataGetFromDate(date('Y-m-d H:00:00', time()-3*60*60)) ){

            process($station_data,$soap_station,$ch_keys,$ch_value,$zdbh,$stations_ids,$station,$chs_by_culture, time()-3*60*60,1);

        } elseif($station_data = &$soap_station->PesslSOAPStationDataGetFromDate(date('Y-m-d H:00:00', time()-12*60*60)) ){

            process($station_data,$soap_station,$ch_keys,$ch_value,$zdbh,$stations_ids,$station,$chs_by_culture, time()-12*60*60,1);
